...
/// Balance, exposure, the CDL and contrast: the part of a primary that reaches
/// a pixel without consulting the hue curves.
///
/// Factored out because a source grade and a clip's own grade both run it, and
/// two copies of this order is two chances for them to drift apart.
void applyTone(const GradeConstants& grade, float& r, float& g, float& b) {
...
/// Saturation about a pixel's own luma, with nothing shaping it by hue.
///
/// The clip's own saturation is entangled with the hue curves -- one multiplier
/// carries both -- but a source grade has no hue curves, so it needs the plain
/// version rather than a copy of that machinery with the curves switched off.
void applySaturation(float amount, float& r, float& g, float& b) {
    if (amount == 1.0F) {
        return;
    }
...
void gradePixel(const GradeConstants& grade, float& r, float& g, float& b, const CurveTable* curves,
                ...
                const ColorCurveTable* hue, const GradeConstants* source, const LutTable* sourceLut,
                float sourceLutAmount) {
    // The source stage first, and in this order: the input LUT is what gets a
    // camera's log onto a curve, and a grade built on the file is built on what
    // that LUT leaves. Everything below here is the clip's own grade, sitting
    // on a picture the file has already answered for.
    if (sourceLut != nullptr && sourceLut->isValid()) {
        sourceLut->apply(r, g, b, sourceLutAmount);
    }
    if (source != nullptr && !source->isIdentity()) {
        applyTone(*source, r, g, b);
        applySaturation(source->saturation, r, g, b);
    }

    applyTone(grade, r, g, b);
...
void gradeImage(const GradeConstants& grade, RgbaImage& image, const CurveTable* curves,
    ...
    const bool fromSource =
        (source != nullptr && !source->isIdentity()) ||
        (sourceLut != nullptr && sourceLut->isValid() && sourceLutAmount > 0.0F);
    if ((grade.isIdentity() && !curved && !keyed && !looked && !shapedByHue && !fromSource) ||
        !image.isValid()) {
        return;
    }
    for (std::int32_t y = 0; y < image.height(); ++y) {
        ...
        for (std::int32_t x = 0; x < image.width(); ++x) {
            ...
            gradePixel(grade, r, g, b, curves, secondary, lut, lutAmount, hue, source, sourceLut,
                       sourceLutAmount);
            pixel.r = r * alpha;
            pixel.g = g * alpha;
            pixel.b = b * alpha;
